...
> ⚠️ **关于 Discourse 用户 API Key**：代码中保留了 `LINUXDO_API_KEY` 支持及
> `tools/generate_api_key.py` 生成工具，但 **linux.do 已在站点设置中禁用用户 API Key**
> （授权页会提示"此功能可能被网站管理员禁用"），因此该路线在 linux.do 上不可用，
> 仅适用于开放了此功能的其他 Discourse 站点。
